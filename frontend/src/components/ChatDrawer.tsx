        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className={`text-xs mt-0.5 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>
              Proactive Productivity Partner
            </p>
          </div>
        </div>
      ...
      <div className={`flex-1 overflow-y-auto p-5 space-y-4 ${
        ...
        {chatHistory.length === 0 ? (
          <div className="text-center py-16 text-slate-400 max-w-xs mx-auto">
            ...
            <h4 className={`text-sm font-bold ${theme === 'dark' ? 'text-slate-200' : 'text-slate-800'}`}>Life Saver Agent</h4>
            <p className={`text-sm mt-2 leading-relaxed ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`}>
                    ...
                    <div className="flex flex-col gap-1.5 pt-1">
                      ...
                      {message.suggestedActions.map((actionOpt, actIdx) => (
                        <button
                          ...
                          className={`w-full text-left py-2.5 px-3 rounded-lg border text-sm font-extrabold transition-all flex items-center justify-between group cursor-pointer ${
                            theme === 'dark'
                        ...
                        >
                          <span className="flex items-center gap-2 truncate">
                            {actionOpt.action === 'autoSchedule' && <Calendar className="w-4 h-4 shrink-0" />}
                            {actionOpt.action === 'createSubtasks' && <CheckSquare className="w-4 h-4 shrink-0" />}
                            {actionOpt.action === 'generateDraft' && <FileText className="w-4 h-4 shrink-0" />}
