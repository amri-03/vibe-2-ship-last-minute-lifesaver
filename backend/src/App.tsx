...
export default function App() {
  ...
  // Guest login handler
  const handleGuestLogin = (customUser?: { displayName: string; email: string }) => {
    const guestUser = {
      ...
      <aside className={`hidden md:flex flex-col border-r fixed top-0 left-0 h-screen justify-between shrink-0 transition-all duration-300 z-35 backdrop-blur-md ${
        ...
        <div className="space-y-8 w-full animate-fade-in">
          ...
          <div className={`flex items-center ${isSidebarCollapsed ? 'justify-center w-full' : 'gap-3'}`}>
            <div
              className={`w-14 h-14 rounded-2xl flex items-center justify-center border transition-all shrink-0 p-1.5 ${
                theme === 'dark'
                  ? 'bg-indigo-600 border-indigo-500 shadow-[0_0_15px_rgba(79,70,229,0.3)]'
                  : 'bg-indigo-600 border-indigo-500 shadow-md shadow-indigo-650/20'
              }`}
            >
            ...
            {!isSidebarCollapsed && (
              <div className="flex-1 min-w-0">
                <h1 className={`text-base font-black tracking-tight ${theme === 'dark' ? 'text-white' : 'text-slate-800'}`}>
                  Life Saver
                </h1>
                <div className="flex flex-col gap-0.5">
                  <span className="text-xs font-extrabold text-emerald-500 uppercase tracking-widest block">
                    ACTIVE SHIELD
                  </span>
                  <span className={`text-xs font-extrabold uppercase tracking-widest block font-mono ${
                    aiEngine === 'gemini' ? 'text-indigo-400' : 'text-amber-500'
                  }`}>
          ...
          {/* Navigation Menu (Dashboard, Calendar, Settings) */}
          <nav className={`space-y-3 ${isSidebarCollapsed ? 'w-full flex flex-col items-center' : ''}`}>
            <button
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all border cursor-pointer ${
                isSidebarCollapsed ? 'w-11 h-11 p-0 justify-center' : 'w-full'
              } ${
            ...
            >
              <div className="relative">
                ...
                {isSidebarCollapsed && calendarEvents.length > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-indigo-500 text-white text-[10px] rounded-full flex items-center justify-center font-black">
        ...
        <div className={`space-y-4 pt-4 w-full ${
          ...
          {/* User profile identifier block instead of clock timer */}
          {!isSidebarCollapsed ? (
            <div className="flex items-center gap-2.5 px-1 py-0.5">
              <button
                onClick={() => setIsSidebarCollapsed(true)}
                className="w-8 h-8 rounded-full bg-indigo-600/10 border border-indigo-500/20 flex items-center justify-center text-sm font-black text-indigo-400 hover:bg-indigo-600 hover:border-indigo-500 hover:text-white hover:scale-110 transition-all cursor-pointer shrink-0"
            ...
            <button
              ...
              className="w-10 h-10 rounded-full bg-indigo-600/10 border border-indigo-500/25 flex items-center justify-center text-sm font-black text-indigo-400 hover:bg-indigo-600 hover:border-indigo-500 hover:text-white hover:scale-110 transition-all cursor-pointer shadow-[0_0_12px_rgba(79,70,229,0.25)]"
          ...
          <div className={`absolute top-0 left-0 w-64 h-full p-6 flex flex-col justify-between shadow-2xl transition-all duration-300 ${
            ...
            <div className="space-y-8">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  <div className="w-12 h-12 rounded-xl bg-indigo-600 flex items-center justify-center p-1 border border-indigo-500/10">
                    <Logo className="w-full h-full text-white" />
                  </div>
              ...
              <nav className="space-y-2">
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-sm font-bold border transition-colors cursor-pointer ${
                    isSettingsOpen ? 'bg-indigo-600 border-indigo-500/30 text-white' : theme === 'dark' ? 'bg-transparent border-transparent text-slate-300' : 'bg-transparent border-transparent text-slate-655'
                  }`}
                >
            ...
            <div className="pt-4 border-t border-slate-200 dark:border-white/[0.04] space-y-3">
              <div className="flex items-center gap-2.5 px-1 py-0.5">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold truncate text-white">{userProfile?.name || 'Active User'}</p>
        ...
        <div className="flex-1 flex flex-row min-w-0 overflow-hidden relative">
          ...
          {activeView === 'dashboard' && (
            <div className="max-w-7xl mx-auto w-full space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6" id="active-workspace-grid">
                <TaskPanel 
                  theme={theme}
                  tasks={tasks}
        ...
        <div className="fixed inset-0 bg-black/85 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-fade-in" id="offline-alert-modal">
          <div className={`w-full max-w-md border rounded-2xl p-6 relative transition-all duration-300 ${
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <h2 className="text-lg font-bold mb-2">Offline Login Acknowledgment</h2>
              <p className="text-sm md:text-base text-slate-300 mb-6 leading-relaxed">
                Google Calendar sync is disabled for local accounts. You can still manage your tasks and productivity calendar offline.
              </p>
              <button
                onClick={() => {
                ...
                className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-base rounded-xl transition-all shadow-md shadow-indigo-600/15 cursor-pointer"
              >
                Agree
              </button>
            </div>
